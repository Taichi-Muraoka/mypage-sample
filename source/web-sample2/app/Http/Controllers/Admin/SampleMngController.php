 ...
 * サンプル管理 - コントローラ
 */
class SampleMngController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('pages.admin.sample_mng', [
    ...
    {
        // targetで "#modal-dtl" が取れるので、複数モーダルがある場合は、modal_idを利用してreturnするデータを決める
        //$this->debug($request->input('target'));
        ...
        // MEMO: SQLログを出力する場合 $this->debug(\DB::getQueryLog())とセットで使用する
        // SQLの表示
        //\DB::enableQueryLog();
        ...
        $sampleData = $query
            ...
            ->where('sample_id', $id)
            // 生徒名取得
            // JOINは必ずモデルを通じで行う。Studentモデルを使用する。
            ->sdLeftJoin(Student::class, function ($join) {
                $join->on('samples.student_id', '=', 'students.student_id');
            })
            // ステータス取得
        ...
        // MEMO: SQLログを出力する場合 \DB::enableQueryLog()とセットで使用する
        // SQLの表示
    ...
    public function new()
    {
        ...
        return view('pages.admin.sample_mng-input', [
