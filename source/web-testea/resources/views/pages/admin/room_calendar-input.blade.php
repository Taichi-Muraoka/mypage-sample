...
@section('title', (request()->routeIs('room_calendar-calendar-edit')) ? '授業スケジュール編集' : '授業スケジュール登録')
...
<x-bs.card :form=true>
    ...
    <p>授業スケジュールの{{(request()->routeIs('room_calendar-edit')) ? '変更' : '登録'}}を行います。</p>
    ...
    <x-input.select caption="状態" id="status" :select2=true :editData="$editData">
    ...
    <x-input.select caption="授業代講" id="daiko" :select2=true :editData="$editData">
    ...
    {{-- 編集時 --}}
    <x-input.select caption="当日欠席" id="todayabsent" :select2=true :editData="$editData">
