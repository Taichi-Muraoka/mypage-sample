...
<x-bs.table :hover=false :vHeader=true>
    ...
    {{-- v-showは、コース種別によって非表示の場合があるため --}}
    <tr v-Show="item.course_kind && (item.course_kind == {{ App\Consts\AppConst::CODE_MASTER_42_1 }} || item.course_kind == {{ App\Consts\AppConst::CODE_MASTER_42_2 }})">
        <th>通塾</th>
        <td>@{{item.how_to_kind_name}}</td>
    </tr>
    <tr v-Show="item.course_kind && (item.course_kind != {{ App\Consts\AppConst::CODE_MASTER_42_3 }} && item.substitute_kind != {{ App\Consts\AppConst::CODE_MASTER_34_0 }})">
        <th>授業代講</th>
        <td>@{{item.substitute_kind_name}}</td>
    </tr>
    {{-- v-showは、コース種別によって非表示の場合があるため --}}
    <tr v-Show="item.course_kind && (item.course_kind == {{ App\Consts\AppConst::CODE_MASTER_42_1 }})">
        <th>出欠ステータス</th>
