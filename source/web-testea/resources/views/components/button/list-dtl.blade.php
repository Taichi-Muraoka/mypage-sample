...
<button type="button" 
  class="btn btn-sm @if (empty($btn)){{ 'btn-secondary' }}@else{{ $btn }}@endif @if (!empty($class)){{ $class }}@endif" 
  data-toggle="modal"
...
{{-- vue用のbindを使用 --}}
<a :href="{{ $vueHref }}" 
  class="btn btn-secondary btn-sm @if (!empty($class)){{ $class }}@endif" role="button">
  @if (empty($caption)){{ '詳細' }}@else{{ $caption }}@endif
</a>
...
{{-- 単純なリンクの遷移 --}}
<a href="@if (empty($href)){{ '#' }}@else{{ $href }}@endif" class="btn btn-secondary btn-sm @if (!empty($class)){{ $class }}@endif" role="button">
  @if (empty($caption)){{ '詳細' }}@else{{ $caption }}@endif
</a>
