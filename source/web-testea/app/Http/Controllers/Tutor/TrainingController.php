...
class TrainingController extends Controller
{
    ...
    public function search(Request $request)
    {
        ...
        $trainings = $query
            ->select(
                ...
                'code_master.name AS trn_type_name',
                'text',
                'release_date',
                'limit_date',
            ...
            ->sdLeftJoin(TrainingBrowse::class, function ($join) use ($account_id) {
                $join->on('training_browse.trn_id', '=', 'training_contents.trn_id')
                    ->where('training_browse.tid', '=', $account_id);
            })
            // 形式名を取得
            ->sdLeftJoin(CodeMaster::class, function ($join) {
                $join->on('training_contents.trn_type', '=', 'code_master.code')
    ...
    public function movieBrowse(Request $request)
    {
        ...
        TrainingContents::where('trn_id', '=', $trn_id)
    ...
    public function download($trnId)
    {
        ...
        // 公開日前・期限日を過ぎているものはエラーを返す
        $training = TrainingContents::select('url')
            ->where('trn_id', '=', $trnId)
