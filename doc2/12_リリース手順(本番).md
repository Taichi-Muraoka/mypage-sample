...
- [リリース手順(本番)](#リリース手順本番)
  - [概要](#概要)
  - [js/cssのビルド・ビルドファイル格納（ローカル作業 ）](#jscssのビルドビルドファイル格納ローカル作業-)
...
ローカル環境での修正確認後、開発サーバのソース最新化を行い、  
開発サーバでも動作確認を行ってください。  
（障害対応の場合は、hotfix-mergeブランチに切り替えてpullする）  

**ローカルでの修正・確認→マージ（developまたはhotfix-merge）→開発サーバでの動作確認→リリース作業**
...
格納後にpublic_buildフォルダ内を add/commit/pushする。
(リリースしたものを管理するため)
...
GitHubのCreate a new releaseをクリック（初回）
2回目以降は、Releases をクリックし、Draft a new releaseボタン押下。
...
作業時にBasic認証をかけたほうがよいかも。PJで決めておく。
Basic認証をかける場合、public配下の .htaccess ファイルを編集する。
テスティ―用には、.htaccess.release を用意しているのでそれを使ってください。
ユーザー名・パスワードは戸羽に聞いてください。

```
cd /home/greenrabbit44/git/testea-mypage/source/web-testea/public
cp -p .htaccess.release .htaccess
```
